...
# def calculate_embedding_distance(model, tokenizer, context, completion):
#     full_text = context + completion
#     with torch.no_grad():
#         inputs_state = tokenizer(full_text, return_tensors="pt").to(model.device)
#         outputs_ans = model(**inputs_state, output_hidden_states=True)
#         norm1_diff,norm2_diff = 0.0, 0.0
#         prev_state = outputs_ans.hidden_states[0].mean(dim=1)
#         for curr_state in outputs_ans.hidden_states[1:]:
#             _diff = torch.norm(curr_state.mean(dim=1) - prev_state, p=1, dim=-1).item()
#             norm1_diff += _diff
#             norm2_diff += 1-torch.nn.functional.cosine_similarity(curr_state.mean(dim=1), prev_state).item()
#             prev_state = curr_state.mean(dim=1).clone()
#         norm1_diff = norm1_diff / len(outputs_ans.hidden_states)
#         norm2_diff = norm2_diff / len(outputs_ans.hidden_states)

#     return 0, norm1_diff, norm2_diff

def calculate_semantic_surprisal(model, tokenizer, context, completion):
    """
    ...
    with torch.no_grad():
        ...
        l1_norm = torch.norm(
            pre_answer_state - post_answer_state, p=1
        ).item()
...
def compute_trajectory_metrics(model, tokenizer, prompt, response, answer_content, own_answer):
    ...
    own_completion = "\nThe answer is " + own_completion
    completion = "\nThe answer is " + answer_content
    # completion = "\n" + steps[-1]
    for step in steps:
        current_context += step + "\n"
        d_prob = calculate_conditional_perplexity(model, tokenizer, current_context, completion)
        ppl_to_own = calculate_conditional_perplexity(model, tokenizer, current_context, own_completion)
        # _, diff1, diff2 = calculate_embedding_distance(model, tokenizer, current_context, completion)
        # ctx_cos, l1_norm = calculate_semantic_surprisal(model, tokenizer, current_context, completion)
        
        traj_prob.append(d_prob)
        # traj_emb.append(diff1)
        # traj_ent.append(diff2)
        traj_ctx_l1.append(d_prob)
        traj_ctx_cos.append(ppl_to_own-d_prob)
...
def process_metrics_and_balance(model, tokenizer, collected_data):
    ...
    for idx, item in enumerate(collected_data):
        ...
        for r_idx in tqdm(selected_indices, desc="Trajectories", leave=False):
            ...
            own_answer = parse_prediction(r, gt, 'math')
            final_answer = gt
            
            metrics = compute_trajectory_metrics(model, tokenizer, prompt, r, final_answer, own_answer)
...
if __name__ == "__main__":
    ...
    data_file = os.path.join(os.path.dirname(__file__), "new_collected_data.pkl")
    data = []
    ...
    results, all_metrics = process_metrics_and_balance(model, tokenizer, data[:20])
