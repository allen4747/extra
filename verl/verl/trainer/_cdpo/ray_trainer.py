...
class CuriosityMemory:
    ...
    def __init__(
        self,
        pad_token_id: int,
        max_rollouts_per_prompt: int = 128,
        max_prefixes_per_prompt: int = 512,
    ):
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.curiosity_memory = CuriosityMemory(
            ...
            max_rollouts_per_prompt=int(self._cfg_get("algorithm.curiosity.max_rollouts_per_prompt", 128)),
    ...
    def _process_thoughts(self, resp: str) -> list[str]:
    ...
    def _compute_prefix_embeddings(
        ...
        emb_outputs = self.actor_rollout_wg.compute_embedding(prefix_batch)
        return emb_outputs.batch["embeddings"].detach().cpu()

    def _update_prefix_memory(self, batch: DataProto, entropy_mat: torch.Tensor) -> None:
        if entropy_mat is None:
            return
        ...
        grouped_prefixes: dict[tuple[int, ...], list[tuple[list[int], float]]] = defaultdict(list)

        for i in range(prompts.shape[0]):
            prompt_key = self._prompt_key(prompts[i])
            valid_len = int(response_mask[i].sum().item())
            if valid_len <= 0:
                continue

            response_ids = responses[i][:valid_len]
            response_text = self.tokenizer.decode(response_ids, skip_special_tokens=True)
            steps = self._process_thoughts(response_text)
            if len(steps) == 0:
                continue
            ...
            for step in prefix_steps:
                ...
                prefix_entropy_mean = float(entropy_mat[i, :prefix_len].mean().item())
                grouped_prefixes[prompt_key].append((prefix_ids, prefix_entropy_mean))

        for prompt_key, items in grouped_prefixes.items():
            prefix_token_lists = [x[0] for x in items]
            prefix_embeddings = self._compute_prefix_embeddings(prompt_key, prefix_token_lists)
            for (prefix_ids, prefix_entropy_mean), emb in zip(items, prefix_embeddings, strict=True):
                self.curiosity_memory.add_prefix_entry(prompt_key, prefix_ids, prefix_entropy_mean, emb)
    ...
    def _enqueue_guided_prompts(self, hard_prompt_keys: list[tuple[int, ...]]) -> None:
        ...
        max_queue = int(self._cfg_get("algorithm.guided_resampling.max_queue_size", 8192))
    ...
    def _dequeue_guided_prompts_for_regen(self) -> list[list[int]]:
        if not self.guided_resampling_enabled:
            return []
        ...
        if len(self._guided_prompt_queue) < regen_batch_size:
            return []

        prompts = self._guided_prompt_queue[:regen_batch_size]
        self._guided_prompt_queue = self._guided_prompt_queue[regen_batch_size:]
        return prompts

    def _build_guided_regen_gen_batch(self, prompt_token_lists: list[list[int]]) -> DataProto:
        assert len(prompt_token_lists) > 0
        pad_id = self.tokenizer.pad_token_id
        max_len = max(len(x) for x in prompt_token_lists)

        bsz = len(prompt_token_lists)
        input_ids = torch.full((bsz, max_len), pad_id, dtype=torch.long)
        attention_mask = torch.zeros((bsz, max_len), dtype=torch.long)
        position_ids = torch.zeros((bsz, max_len), dtype=torch.long)

        for i, ids in enumerate(prompt_token_lists):
            seq_len = len(ids)
            if seq_len == 0:
                continue
            input_ids[i, :seq_len] = torch.tensor(ids, dtype=torch.long)
            attention_mask[i, :seq_len] = 1
            position_ids[i, :seq_len] = torch.arange(seq_len, dtype=torch.long)

        raw_prompt_ids = np.array([np.array(x, dtype=np.int64) for x in prompt_token_lists], dtype=object)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if len(guided_prompt_lists) > 0:
                        with marked_timer("guided_regen_gen", timing_raw, color="magenta"):
                            guided_gen_batch = self._build_guided_regen_gen_batch(guided_prompt_lists)
                            guided_gen_batch.meta_info = {
                                "eos_token_id": self.tokenizer.eos_token_id,
                                "pad_token_id": self.tokenizer.pad_token_id,
                                "recompute_log_prob": self.config.actor_rollout_ref.actor.get(
                                    "recompute_log_prob", False
                                ),
                                "global_steps": self.global_steps,
                            }
                            guided_gen_batch = guided_gen_batch.repeat(
                                repeat_times=self.config.actor_rollout_ref.rollout.n,
                                interleave=True,
                            )
                            if not self.async_rollout_mode:
                                guided_gen_batch_output = self.actor_rollout_wg.generate_sequences(guided_gen_batch)
                            else:
                                guided_gen_batch_output = self.async_rollout_manager.generate_sequences(
                                    guided_gen_batch
                                )
                            if "timing" in guided_gen_batch_output.meta_info:
                                guided_timing = {
                                    f"guided_regen/{k}": v
                                    for k, v in guided_gen_batch_output.meta_info["timing"].items()
                                }
                                timing_raw.update(guided_timing)
                                guided_gen_batch_output.meta_info.pop("timing", None)
                        metrics["exploration/guided_regen_batch_size"] = len(guided_prompt_lists)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.curiosity_enabled or self.guided_resampling_enabled:
                            with marked_timer("exploration", timing_raw, color="purple"):
                                ...
                                rollout_embeddings = emb_outputs.batch["embeddings"]
                                entropy_mat = emb_outputs.batch.get("entropys", policy_entropys)
                                ...
                                if self.guided_resampling_enabled:
                                    self._update_prefix_memory(batch, entropy_mat)
