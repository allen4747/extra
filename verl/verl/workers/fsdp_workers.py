...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_compute_embedding")
    @torch.inference_mode()
    def compute_embedding(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, embeddings = self.actor.compute_log_prob(data=data, calculate_entropy=True, get_embedding=True)
                # breakpoint()
            output = DataProto.from_dict(
                tensors={"old_log_probs": output, "entropys": entropys, "embeddings": embeddings},
                meta_info={"temperature": self.config.rollout.temperature},
            )
            # TODO: Initialize an embedding model and return the embeddings only
