#!/usr/bin/env bash
# GRPO baseline on Qwen3-1.7B — 4 GPUs, MATH-500 eval, fast iteration.

export CUDA_VISIBLE_DEVICES=0,1,2,3

MODEL_PATH="${MODEL_PATH:-Qwen/Qwen3-1.7B}"
TRAIN_FILE="${TRAIN_FILE:-$HOME/data/math_dapo/train.parquet}"
VAL_FILE="${VAL_FILE:-$HOME/data/aime24/test.parquet}"
EXP_NAME="01_GRPO_Baseline_Qwen3"

python3 -m recipe.extra.main_extra \
  ...
  trainer.default_local_dir="${CHECKPOINT_DIR:-./checkpoints}/ExTra_Qwen/$EXP_NAME" \
  "$@"
