...
#
# Compute note: 8B model needs FSDP offloading and a smaller micro-batch than
# the 1.5B / 1.7B baselines. Rollout GPU memory is capped low so the vLLM
# engine can share device memory with the FSDP-wrapped policy.
#
# Usage:
#   bash verl/examples/experiments/ExTra_Rebuttal/01_grpo_nano8b.sh
#
# Env-var overrides (for retargeting on remote nodes without editing):
