#!/usr/bin/env bash
# Ablation: Regeneration ONLY (no curiosity reward)
# Completes the 2x2 ablation grid: {curiosity, regen} x {on, off}
...
# Occupy a GPU by running a placeholder tensor allocation in the background
occupy_gpu() {
...
while true; do
    if [ -z "$GPU1" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            sleep 2  # let the holder allocate memory
        else
            GPU1=""
    ...
    # Look for a second free GPU (different from GPU1)
    GPU2=$(find_free_gpu)
...
if [ -n "$HOLD_PID" ]; then
    ...
    sleep 2  # let GPU memory free up
fi
...
EXP_NAME="ExTra-RegenOnly-JustRL-Qwen2.5-1.5B"
...
python3 -m verl.trainer.main_ppo \
  ...
  algorithm.curiosity.novelty_reward_scale=0.0 \
