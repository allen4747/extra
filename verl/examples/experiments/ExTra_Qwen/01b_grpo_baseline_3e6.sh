#!/usr/bin/env bash
# GRPO baseline on Qwen3-1.7B with 3e-6 lr — 4 GPUs, matches ExTra runs' learning rate.
# Waits for GPUs 0-3 to become free before launching.

# Wait for GPUs 0-3 to be available (< 1GB used)
echo "Waiting for GPUs 0,1,2,3 to become available..."
while true; do
  all_free=true
  ...
  if [ "$all_free" = true ]; then
    echo "GPUs 0-3 are free. Launching experiment."
    break
  fi
  sleep 30
done
...
EXP_NAME="01b_GRPO_Baseline_3e6_Qwen3"
